...
Need to work on this... figure out why level and var are converting to character, then the rest of the code should be fine.
```{r}
# pop_by_age_out %>% 
# 	mutate(level = as.factor(level), var = as.factor(var) %>% 
# 				 	fct_relevel(var, "under_5_years", "5_to_9_years")) %>% 
# 	select(-share, -moe, -sharemoe) %>% 
# 	group_by(level, geoid, county, var) %>% 
# 	arrange(name, year, var) %>% 
# 	mutate(diff = estimate - lag(estimate, default = first(estimate))) %>% 
# 	arrange(level, geoid, year, var) %>% 
# 	mutate(var = "pop_change") %>% 
# 	select(-estimate, -moe) %>% 
# 	rename(estimate = diff) %>% View()
