...
urban_pal <- c("#1696d2", "#fdbf11", "#d2d2d2", "#ec008b", "#55b748")
...
out$income_ranges <- income_table
...
```{r hh by inc band bar chart, eval=F}
hh_by_inc_band %>% 
	...
	geom_text(aes(
		...
		position = position_dodge(.8), hjust = 1, family = "Lato Regular", size = 4) +
...
```{r hh by inc band share, fig.height=8, fig.width=8}
hh_by_inc_band %>% 
	filter(inc_band != "Total") %>% 
	mutate(inc_band = fct_rev(inc_band)) %>% 
	ggplot(aes(share, inc_band, group = name)) +
	geom_col(aes(fill = inc_band), width = .7, position = position_stack()) +
	scale_x_continuous(expand = expansion(mult = c(0, 0.1)), labels = scales::comma) +
	geom_text(aes(label = percent(share, accuracy = 1)),
						position = position_stack(), hjust = 1, family = "Lato Regular", size = 4) +
...
out$hh_by_race_inc_band <- hh_by_race_inc_band
```
...
hh_by_race_inc_band %>% 
  ungroup() %>% 
	...
	geom_text(aes(label = percent(share, accuracy = 1)),
						position = position_stack(.5), family = "Lato Regular") +
...
```{r pivot race data}
pivot_race <- hh_by_race_inc_band %>% 
	ungroup() %>% 
	select(name, race2, inc_band, value) %>%
	bind_rows(hh_by_inc_band %>% 
							ungroup() %>% 
							select(name, inc_band, value) %>% 
							mutate(race2 = "Total"))

out$pivot_race_inc_band <- pivot_race
```

Here's that chart pivoted so the numerator is the population by race of HOH of a given income band and the denominator is the total population of that income band. This is less helpful for less populous counties.
```{r pivoted race bar, fig.width=10, fig.height=10}
pivot_race %>% 
	select(name, inc_band, race2, value) %>%
	filter(inc_band != "Total") %>%
	ungroup() %>% 
	group_by(name, inc_band) %>% 
	calc_shares(group = race2, denom = "Total", value = value) %>% 
	filter(!is.na(share)) %>% 
	mutate(inc_band = fct_rev(inc_band)) %>% 
	ggplot(aes(share, inc_band, group = race2)) +
	geom_col(aes(fill = race2), width = .8, position = position_stack(1)) +
	guides(fill = guide_legend(title = "", reverse = T)) +
	geom_text(aes(label = percent(share, accuracy = 1)),
						position = position_stack(.5), family = "Lato Regular") +
	...
	scale_fill_custom() +
	labs(x = "", y = "",
			 title = "Share of households by race of HOH in each income band") +
	theme(panel.grid.major  = element_blank(),
				panel.grid.minor = element_blank(),
				axis.text.y = element_text(colour = "black"),
				axis.text.x = element_blank(),
				strip.text.x = element_text(hjust = .5, colour = "black"),
				legend.position = "bottom",
				plot.title.position = "plot")
```
...
**For breakouts by disability type, see disability.Rmd in notebooks folder on repo**
...
ct_inc_band_disability %>%
	...
	geom_text(aes(label = percent(share, accuracy = 1)), position = position_dodge(1), family = "Lato Regular", size = 4.5, hjust = 1.1, vjust = .5) +
	scale_x_continuous(expand = expansion(mult = c(0, 0))) +
	scale_fill_custom(rev = T) +
...
ct_cost_ratio_inc_band_disability %>% 
	...
	geom_text(aes(label = round(avg_cost_ratio, 2)), position = position_dodge(.85), family = "Lato Regular", hjust = .5, vjust = 1.2) +
...
cb2 %>% group_by(level) %>% 
	...
	geom_text(aes(label = percent(share, accuracy = 1)), position = position_stack(.5), family = "Lato Regular", size = 3.75) +
	scale_x_continuous(expand = expansion(mult = c(0, 0))) +
...
out$cb_rates_inc_band <- cb_rates
...
cb_rates %>% 
	...
	geom_text(aes(
		...
		family = "Lato Regular", size = 3.55) +
	scale_x_continuous(expand = expansion(mult = c(.025, .1))) +
	guides(color = guide_legend(title = "")) +
	labs(	x = "", y = "",
		title = "Cost-burden rates for households in each income band",
		caption = "Households spending 30% or more income to housing costs.") +
	scale_color_manual(values = urban_pal) +
	theme(
		panel.grid.minor = element_blank(),
...
```{r cb burden by severity dot plot, fig.height=8, fig.width=10, dpi=300}
cb_by_inc_band %>% 
	ungroup() %>% 
	...
	mutate(name = as.factor(name) %>% 
				 	fct_rev()) %>% 
	ggplot(aes(share, name, group = inc_band)) +
	geom_point(aes(color = inc_band), size = 7, alpha = 0.8) +
	geom_text(aes(
		label = percent(share, accuracy = 1)),
		family = "Lato Regular", size = 2.75) +
	scale_x_continuous(limits = c(0, .75), expand = expansion(mult = c(.025, .025))) +
	facet_grid(rows = "cost_burden", scales = "free", space = "fixed") +
	guides(color = guide_legend(title = "")) +
	labs(
		x = "",
		y = "",
		title = "Shares of Cost-Burdened Households in each Income Band",
	...
	theme(
		...
		strip.text.y = element_text(colour = "black", size = 8.5),
		plot.title = element_text(colour = "black", size = 12),
		legend.text = element_text(colour = "black", size = 8.5),
		plot.caption = element_text(colour = "black", size = 8.5),
...
cb_by_inc_band %>% 
	ungroup() %>% 
	filter(cost_burden == "Severely cost-burdened") %>% 
	group_by(level) %>% 
	mutate(name = as.factor(name) %>% 
				 	fct_rev()) %>% 
	ggplot(aes(share, name, group = inc_band)) +
	...
	guides(color = guide_legend(title = "")) +
	labs(	x = "", y = "",
		title = "Severe cost-burden rates for households in each income band",
		caption = "Households spending 50% or more income to housing costs.") +
	scale_color_manual(values = urban_pal) +
	theme(
		panel.grid.minor = element_blank(),
		panel.grid.major.x = element_blank(),
...
avg_cost_ratio_inc_band %>% 
	...
	geom_text(aes(
		...
		family = "Lato Regular", size = 2.75, alpha = 0.8) +
...
avg_cost_inc_band %>% 
	...
	geom_text(aes(
		...
		family = "Lato Regular", size = 3, alpha = 0.8) +
...
out$affordable_cost_table <- affordable_cost_table %>% 
				select(Name = name, `Very low`:High)
...
out$units_by_cost_and_income <- units_by_cost_income
```
...
units_by_cost_income %>% 
	...
	geom_text(aes(label = comma(value, accuracy = 1)), position = position_dodge(.85), vjust = "top", family = "Lato Regular", size = 3.5, fontface = "bold") +
...
ct_relative %>% 
	...
	geom_text(aes(label = percent(share, accuracy = 1)), position = position_stack(.5), family = "Lato Regular") +
	scale_x_continuous(expand = expansion(mult = c(0, 0))) +
	scale_fill_custom() +
...
out$hh_cost_band_vs_units_cost_band <- income_vs_cost_bands

income_vs_cost_bands %>% 
	mutate(inc_band = fct_rev(inc_band)) %>% 
	ggplot(aes(cost_band, inc_band, fill = units)) +
	geom_tile(show.legend = F) +
	geom_text(aes(label = comma(units)), family = "Lato Regular") +
...
units_by_tenure_occupancy_cost_band <- bind_rows(occ_cost_bands, vac_cost_bands) %>%
	...
	mutate(occupancy = as.factor(occupancy) %>% 
				 	fct_relevel(., "occupied", "vacant-on market, vacant-off market"))

ct_units_by_tenure_occupancy_cost_band <- units_by_tenure_occupancy_cost_band %>% 
	select(-name) %>% 
	arrange(cost_band) %>% 
	group_by(occupancy, tenure, cost_band) %>% 
	summarise(units = sum(units)) %>% 
	mutate(name = "Connecticut")

out$units_by_tenure_occupancy_cost_band <- bind_rows(units_by_tenure_occupancy_cost_band, ct_units_by_tenure_occupancy_cost_band)
```
...
out$units_by_tenure_occupancy_cost_band %>% 
	filter(occupancy != "vacant-off market") %>% 
	ungroup() %>% 
	select(-tenure) %>% 
	group_by(name, occupancy, cost_band) %>% 
	summarise(units = sum(units)) %>% 
	ungroup() %>% 
	mutate(occupancy = as.character(occupancy),
				 occupancy = str_to_sentence(occupancy)) %>% 
	mutate(occupancy = as.factor(occupancy)) %>% 
	mutate(cost_band = as.character(cost_band),
				 cost_band = str_replace(cost_band, "_", " "),
				 cost_band = str_replace(cost_band, "mid ", "mid-"),
				 cost_band = str_to_sentence(cost_band)) %>% 
	mutate(cost_band = as.factor(cost_band) %>% 
				 	fct_relevel(., "Very low", "Low", "Mid-low", "Mid-high", "High")) %>% 
	ggplot(aes(cost_band, units, group = cost_band)) +
	geom_col(aes(fill = occupancy), width = .85) +
  geom_text(aes(label = comma(units, accuracy = 1)),
  					family = "Lato Regular", size = 3,
  					position = position_stack(.5)) +
	scale_y_continuous(labels = comma_format(),
										 expand = expansion(mult = c(0, .15))) +
	scale_fill_manual(values = c(urban_pal[1], urban_pal[2])) +
	guides(fill = guide_legend(title = "")) + 
	...
	theme(plot.title.position = "plot",
				...
				panel.grid.minor = element_blank(),
				panel.grid.major.x = element_blank(),
				axis.text.x = element_text(colour = "black", size = 9,  angle = -45, hjust= 0),
				axis.text.y = element_text(colour = "black", size = 9),
				strip.text.x = element_text(colour = "black", hjust = .5, size = 11),
				legend.position = "bottom") +
	labs(title = "Total Housing Units by Cost Band",
			 x = "", y = "")

ggsave(filename = "../output_data/corrected_charts/units_by_cost_band.png", dpi = 300, height = 10, width = 10)
ggsave(filename = "../output_data/corrected_charts/units_by_cost_band.svg", dpi = 300, height = 10, width = 10)
```
...
ggsave(filename = "../output_data/corrected_charts/units_by_cost_band_no_labels.png", dpi = 300, height = 10, width = 9)
ggsave(filename = "../output_data/corrected_charts/units_by_cost_band_no_labels.svg", dpi = 300, height = 10, width = 9)
```
...
all_units_by_inc_band <- units_by_tenure_occupancy_cost_band %>% 
  select(-tenure, -occupancy) %>% 
	...
	mutate(band = as.character(band),
				 band = str_replace(band, "_", " "),
				 band = str_replace(band, "mid ", "mid-"),
				 band = str_to_sentence(band)) %>% 
...
units_and_hh %>% 
	...
	geom_col(aes(fill = group), position = position_dodge(.7), width = .7, color = "white", size = .25) +
	scale_y_continuous(labels = comma_format()) +
	facet_wrap(facets = "name", scales = "free_y") +
	scale_fill_manual(values = c(urban_pal[1], urban_pal[2])) +
	labs(title = "Comparison of Housing Need and Supply by Income and Housing Cost Band, 2018",
	...
	theme(plot.title.position = "plot",
				...
				legend.position = "bottom",
				axis.text.x = element_text(colour = "black", size = 9, angle = -60, hjust = 0, vjust = .5))

ggsave(filename = "../output_data/corrected_charts/comparison_needs_cost.png", dpi = 300, width = 10)
ggsave(filename = "../output_data/corrected_charts/comparison_needs_cost.svg", dpi = 300, width = 10)

out$units_and_households <- units_and_hh
```

```{r}
cwi::batch_csv_dump(out, path = "../output_data/hh_desiring_housing")
```
