...
# People hate these colors, but mostly because it's not colorblind friendly, but it's not what we're using in the final so I'm not gonna mess with it
pal <- rev(LaCroixColoR::lacroix_palette(name = "Pamplemousse", n = 5, type = "discrete"))
...
**Add basic count of how many cost burdened hh's in CT**
...
**Another question might be, what are the cost ratios for those who are cost burdened? What's the dollar amount over cost-burden that these households are paying?**
...
Some future iteration of this notebook should find a way to combine this table with the chart above, but the topline is that the average actual costs in low and very low income households exceeds the affordable threshold (except low income households in Windham County). The middle-to-high income households' average actual housing cost is within the affordable range.
...
There are many more higher income households than units in that cost band, so high income households are occupying housing affordable to lower-income households. Likewise there are many more very low income households than units and they're occupying more expensive housing than they can afford. There are not enough units affordable to very low income households anywhere, but at the low income threshold, units occupied within that cost band begin to exceed households in that income band.
...
Knowing that households paying into a cost band are not a subset of households in that income band, here's the share of households paying into the appropriate/same cost band, those paying less, and those paying more.
...
Establishing that there's a need for housing in lower cost bands, how many units exist in each of these bands?

*Notes for myself: Total up occupied units (renter + owner, any reason to separate?) and vacant units available in each cost band. -- DONE*
*Vacant by contract rent and mortgage payment estimate generated using some average values for CT.*
*Need a new PUMS file in hierarchical format, widdled down to household level.*
...
rent_vac_cost_bands <- vac_des %>%
	filter(owncost == "99999") %>% 
	select(hhwt, name, rent, vl, l, ml, mh) %>% 
	group_by(name) %>% 
	mutate(tenure = "renter_vacant",
...
#right now these are all ct averages, not county
##testing process to calculate this... will be a million mutates
int_rate <- .04179 #HMDA: statewide average for first lien mortgages in 2019 was 4.179
pmi <- .075 #avg mortgage insurance rate between 5 and 10% of loan amount
median_mill <- 31.17 #statewide median mill rate
valueh <- 200000 #example cheap house in CT

loan = .9 * valueh
month_mortgage= (int_rate / 12) / 100
monthly_PI = loan * month_mortgage * ((1+month_mortgage)**360)/(((1+month_mortgage)**360)-1)

PMI = (.075 * loan ) / 12
property_tax = loan / 1000 * .7 * median_mill / 12
total_month = monthly_PI + PMI + property_tax


#leah had taxes at 25% of mortgage payment but I'm not sure about that. Isn't it based on property value, not mortgage payment?
```
