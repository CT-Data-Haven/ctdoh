...
```{r units by tenure and occ bar chart, fig.width=10, fig.height=10}
out$units_by_tenure_occupancy_cost_band %>% 
  ...
  geom_text(aes(label = comma(units, accuracy = 1)),
	...
	scale_fill_manual(values = c(urban_pal[1], urban_pal[3], urban_pal[5])) +
	guides(fill = guide_legend(title = "")) + 
...
units_and_hh %>% 
	...
	geom_col(aes(fill = group), position = position_dodge(.5), color = "white", size = .25) +
	scale_y_continuous(labels = comma_format()) +
	facet_wrap(facets = "name", scales = "free_y") +
	scale_fill_manual(values = c(urban_pal[1], urban_pal[5])) +
