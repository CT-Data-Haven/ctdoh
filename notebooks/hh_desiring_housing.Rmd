...
Pretty sure here I'm going to need a dodged bar chart of occupied units in each income band and occupied units in each cost band.
...
This is interesting, and quite different from the DC study. We have many more high income households than occupied units in that cost band, so high income households are occupying housing affordable to lower-income households. There are not enough units affordable to low income households anywhere.
...
units_by_cost_income %>% 
	...
	geom_col(aes(fill = group), width = .7, position = position_dodge(.75)) +
	geom_text(aes(label = comma(value, accuracy = 1)), position = position_dodge(.75), vjust = "top", family = "Roboto Condensed", size = 3) +
...
Another interesting chart might be the share of households paying into the right cost band, those paying less, and those paying more. Only showing CT but providing a table by county.
...
relative_housing_costs <- units %>%
	...
	mutate(v = if_else(inc_code == cost_code, "same", "higher")) %>%
	mutate(v = if_else(inc_code > cost_code, "lower", v)) %>%
	select(hhwt, name, inc_band, v) %>%
	group_by(name, inc_band, v) %>%
	summarise(value = sum(hhwt))

ct_hhld_totals <- ct_hhlds %>% 
	mutate(v = "total") %>% 
	select(inc_band, v, value)
...
ct_relative <- relative_housing_costs %>% 
	...
	ungroup() %>%
	group_by(inc_band) %>% 
	calc_shares(group = v, denom = "total", value = value) %>% 
	mutate(inc_band = as.factor(inc_band) %>%
				 	fct_relevel("Very low", "Low", "Mid-low", "Mid-high", "High"))

ct_relative %>% 
	ggplot(aes(share, inc_band, group = v)) +
	geom_col(aes(fill = v), position = position_stack())
```
...
According to HMDA data, the statewide average for first lien mortgages in 2019 was 4.179. I'll further disaggregate by county and do additional analysis on high interest loans and rates by race of applicant/other demographic flags, but for now I'll use that value to estimate mortgage payments.
...
int_rate <- 4.179
pmi <- 7.5 #avg mortgage insurance rate between 5 and 10% of loan amount
#leah had taxes at 25% of mortgage payment but I'm not sure about that. Isn't it based on property value, not mortgage payment? Should I take an average of the statewide mill rate? of the county?
```
