...
```{r}
# John
# I am regarding N/A as no disability, but should revisit
...
get_counts <- function(col) {
  ...
  inc_band_disability <- des %>% 
    select(hhwt, inc_band, !!colname) %>% 
  	mutate(name = "Connecticut") %>% 
  	group_by(name, inc_band, !!colname) %>% 
  	summarise(value = survey_total(hhwt)) %>% 
  	ungroup() %>% 
  	group_by(name, inc_band)
...
ct_inc_band_disability_dfs <- pums %>% 
  select(ends_with("TF")) %>% 
  names() %>% 
  lapply(get_counts)
```
...
```{r}
# John
ct_inc_band_disability_dfs %>% lapply(kable)
```
...
#	scale_fill_custom(rev = T) + # temporarily commented because John was running into ggplot errors
