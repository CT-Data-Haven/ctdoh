...
# 7GB raw data split in terminal, stitched together in database, and filtered here.
# raw data are zipped up on black external hd if needed again
...
out <- file_list %>% map(~vroom(file = paste(in_path, ., sep = ""), delim = "|") %>%
										...
										select(state_code, county_code, interest_rate, loan_amount, total_units,
